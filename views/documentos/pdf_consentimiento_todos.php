...
foreach ($this->reservas_cds as $index => $row) {
    ...
    $nombrePaciente = trim($row["nombres"] . ' ' . $row["apellido_paterno"] . ' ' . $row["apellido_materno"]);
    ...
    // $centro = mb_convert_encoding(trim($row["centro"]), 'UTF-8', 'ISO-8859-1');
    // $centro = utf8_decode(trim($row["centro"]));
    ...
    <table class="table" style="border-collapse: collapse; width: 100%; font-size: 12px;">
        ...
        <tr>
            ...
            <td style="width: 60%; border: 0.5px solid black; line-height: 2.5;">FECHA DE ATENCIÓN: <b>$fecha_formateada</b></td>
        </tr>
        <tr>
...
// Salida del PDF
$pdf->Output('consentimiento_todos.pdf', 'I');
