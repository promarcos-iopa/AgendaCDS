...
class InformeModel extends Model
{
    ...
    public function descargar_Excel_pacientes($datos_informe){
        ...
        try {
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('F1', "LENTES INDICADOS");
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('K1', "MEDICO ATENDIO");
            $objPHPExcel->getActiveSheet()->SetCellValue('L1', "PROGRAMA");
            $objPHPExcel->getActiveSheet()->SetCellValue('M1', "TIPO ATENCION");
          

             // Aplicar formato de negrita al encabezado
            $encabezadoEstilo = $objPHPExcel->getActiveSheet()->getStyle('A1:M1');
            ...
            for ($i = 0; $i < $numDetalle; $i++) {
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('F'.$fila, $cantidad_lentes);
                ...
                $vision_lejos_od_lc = isset($data[$i]['sl_vision_lejos_od_m']) ? $data[$i]['sl_vision_lejos_od_m'] : '';
                $vision_lejos_oi_lc = isset($data[$i]['sl_vision_lejos_oi_m']) ? $data[$i]['sl_vision_lejos_oi_m'] : '';
                ...
                $estado = ($recepcionado == 1 && $codigo_pago_cuenta >= 2) ? 'Asistió' : 'Ausente';
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('K'.$fila, $nombre_medico);
                //PROGRAMA
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('L'.$fila, $programa);
                //TIPO ATENCION 
                $objPHPExcel->getActiveSheet()->SetCellValue('M'.$fila, $data[$i]["codigo_tipo_prestacion_agenda"] == 1 ? "CONSULTA" : "CONTROL");
                // //DIAGNOSTICO
                // $objPHPExcel->getActiveSheet()->SetCellValue('N'.$fila,isset($data[$i]["detalle_atencion"]) && $data[$i]["detalle_atencion"] !== null ? $data[$i]["detalle_atencion"] : "");
    ...
    public function descargar_Excel_optica($datos_informe){
        ...
        try {
           ...
           $objPHPExcel->getActiveSheet()->SetCellValue('H1', "EMAIL");//* AGREGADO el 23-12-2024
           $objPHPExcel->getActiveSheet()->SetCellValue('I1', "TELEFONO");//* AGREGADO el 23-12-2024


            // Aplicar formato de negrita al encabezado
        //    $encabezadoEstilo = $objPHPExcel->getActiveSheet()->getStyle('A1:G1');
           $encabezadoEstilo = $objPHPExcel->getActiveSheet()->getStyle('A1:I1');//* ACTUALIZADO EL 23-12-2024
           $encabezadoFuente = $encabezadoEstilo->getFont();
           $encabezadoFuente->setBold(true);
           ...
           for ($i = 0; $i < $numDetalle; $i++) {
               ...
               $objPHPExcel->getActiveSheet()->SetCellValue('G'.$fila,  $data[$i]['centro']);
                ...
                //* INICIO Agregar Columnas Valores de EMAIL y TELEFONO
                $email = $data[$i]["correo_electronico"];
                // Verificar si telefono1 tiene datos
                if (!empty($data[$i]["telefono1"])) {
                    $telefono = $data[$i]["telefono1"];
                }
                // Si telefono1 está vacío, verificar si telefono2 tiene datos
                elseif (!empty($data[$i]["telefono2"])) {
                    $telefono = $data[$i]["telefono2"];
                }
                // Si ninguno de los anteriores tiene datos, verificar telefono3
                elseif (!empty($data[$i]["telefono3"])) {
                    $telefono = $data[$i]["telefono3"];
                }
                // Si ninguno tiene datos, dejar vacío
                else {
                    $telefono = "";
                }
                $objPHPExcel->getActiveSheet()->SetCellValue('H'.$fila, $email);
    ...
    public function buscarDocConsentimiento($rut, $fecha_atencion){
        $archivosPDF = [];
        try{
          $query=$this->db->connect()->query("SELECT * FROM formulario_consentimiento WHERE rut='$rut' AND DATE(fecha_registro) ='$fecha_atencion'");
          while($row=$query->fetch()){
            $archivosPDF[] = [
              'id' => $row['id'],
